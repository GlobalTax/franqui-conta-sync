...
/** Formatea número con coma decimal (estándar español) para CSV */
function fmtES(value: number, decimals = 2): string {
...
export function generateDiarioCSV(config: GeneratorConfig): string {
  ...
  for (let i = 0; i < entryCount; i++) {
    ...
    // Línea débito
    rows.push(`${dateStr};${entry.desc};${entry.debit};${fmtES(amount)};0;${entry.desc} - Debe`);
    // Línea crédito
    rows.push(`${dateStr};${entry.desc};${entry.credit};0;${fmtES(amount)};${entry.desc} - Haber`);
  }
...
export function generateIVAEmitidasCSV(config: GeneratorConfig): string {
  ...
  for (let i = 0; i < count; i++) {
    ...
    rows.push(
      `${fecha};${numeroFactura};${cliente.nombre};${cliente.cif};${fmtES(base)};${tipoIva};${fmtES(cuotaIva)};${fmtES(total)};Venta productos`
    );
  }
...
export function generateIVARecibidasCSV(config: GeneratorConfig): string {
  ...
  for (let i = 0; i < count; i++) {
    ...
    rows.push(
      `${fecha};${numeroFactura};${proveedor.nombre};${proveedor.cif};${fmtES(base)};${tipoIva};${fmtES(cuotaIva)};${fmtES(total)};Compra mercaderías`
    );
  }
