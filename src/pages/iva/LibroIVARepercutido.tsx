...
import { useLibroIVARepercutido } from "@/hooks/useIVABooks";
...
const LibroIVARepercutido = () => {
  ...
  const { data: libroIVA, isLoading } = useLibroIVARepercutido(
    centroCode,
  ...
  const totals = libroIVA?.reduce(
    (acc, row) => ({
      ...
      recargo: acc.recargo + Number(row.recargo_equivalencia),
      total: acc.total + Number(row.total_factura),
    }),
    { base: 0, cuota: 0, recargo: 0, total: 0 }
  );
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        breadcrumbs={[
          { label: "IVA" },
          { label: "Libro de Facturas Expedidas" },
        ]}
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <ExportButton
              printRef={printRef}
              data={exportData}
              filename="libro-iva-repercutido"
            />
          </div>
        </CardHeader>
        <CardContent>
      ...
      <Card ref={printRef}>
        <CardHeader>
          <CardTitle>Libro de Facturas Expedidas (IVA Repercutido)</CardTitle>
          {startDate && endDate && (
        ...
        <CardContent>
          {isLoading ? (
            ...
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                No hay facturas expedidas en el periodo seleccionado
              </AlertDescription>
            </Alert>
          ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">Recargo Equiv.</TableHead>
                ...
                <TableBody>
                  {libroIVA.map((row, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell className="text-right font-mono">
                        {row.recargo_equivalencia > 0
                          ? formatAmount(row.recargo_equivalencia)
                          : "-"}
                      </TableCell>
...
export default LibroIVARepercutido;
